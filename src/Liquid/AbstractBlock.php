...
{
    const TAG_PREFIX = 'Vs\Amp\Liquid\Tag\Tag';
    ...
    public function parse(array &$tokens)
    {
        ...
        while (count($tokens)) {
            ...
            $token = preg_replace('/\\s\\s+/ ', ' ', $token, -1);
            if ($variableStartRegexp->match($token)) {
                $this->whitespaceHandler($token);
                $this->nodelist[] = $this->createVariable($token);
            } elseif ($startRegexp->match($token)) {
                $this->whitespaceHandler($token);
    ...
    protected function renderAll(array $list, Context $context)
    {
        ...
        foreach ($list as $token) {
            ...
            if (is_array($value)) {
                $value = '<pre>('.gettype($value).') '.html::specialchars(print_r($value, true)).'</pre>';
